    ...
    return (
        <div className="bg-surface text-on-surface font-body selection:bg-secondary selection:text-on-secondary min-h-screen flex flex-col overflow-x-hidden">
            ...
            <header className="sticky top-0 z-50 bg-gradient-to-r from-primary via-primary to-primary text-[#fdfbf7] shadow-2xl border-b-2 border-secondary font-sans leading-relaxed">
                ...
                <div className="max-w-screen-2xl w-full mx-auto px-3 lg:px-8 py-3 lg:py-5 relative z-10 flex justify-between items-center bg-primary">
                    ...
                    <div className="flex justify-start items-center gap-3 md:gap-5">
                        <button 
                            className="lg:hidden text-white hover:text-secondary p-1 flex items-center transition-colors" 
            ...
            <div className={`fixed inset-0 z-[100] transition-opacity duration-300 lg:hidden ${isMobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
                ...
                ></div>
                <div 
                    className={`absolute top-0 right-0 max-w-sm w-[85%] h-full bg-surface shadow-2xl flex flex-col transition-transform duration-300 ease-in-out border-l border-secondary/20 ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}
                >
                    <div className="px-6 py-6 border-b border-outline-variant/20 flex justify-between items-center bg-primary">
                        <span className="text-xl font-headline tracking-widest text-secondary font-bold">RADHA MAHAL</span>
                        <button 
                            onClick={() => setIsMobileMenuOpen(false)} 
                            className="text-white hover:text-secondary p-1 transition-colors"
                        >
                    ...
                    <nav className="flex-1 overflow-y-auto py-8 px-6 space-y-6 flex flex-col font-sans">
                        <div className="space-y-4">
                            ...
                            {categories.map(cat => (
                                <Link 
                                    ...
                                    className="block text-on-surface hover:text-secondary font-medium tracking-wide transition-colors"
                                    onClick={() => setIsMobileMenuOpen(false)}
                                >
                                    {cat}
                                </Link>
                            ...
                            {categories.length === 0 && (
                                <Link to="/catalog" className="block text-on-surface hover:text-secondary font-medium tracking-wide" onClick={() => setIsMobileMenuOpen(false)}>View All</Link>
                            )}
                        </div>

                        {occasions && occasions.length > 0 && (
                            <div className="space-y-4 pt-4">
                                <h3 className="text-secondary text-sm font-bold tracking-widest uppercase border-b border-outline-variant/30 pb-2">Collections</h3>
                                {occasions.map(occ => (
                                    <Link 
                                        key={occ} 
                                        to={`/catalog?occasion=${encodeURIComponent(occ)}`} 
                                        className="block text-on-surface hover:text-secondary font-medium tracking-wide transition-colors"
                                        onClick={() => setIsMobileMenuOpen(false)}
                                    >
                                        {occ}
                        ...
                        <div className="space-y-4 pt-4 border-t border-outline-variant/30">
                            <Link to="/catalog?sort=New+Arrivals" className="block text-on-surface hover:text-secondary font-bold tracking-wide transition-colors" onClick={() => setIsMobileMenuOpen(false)}>New Arrivals</Link>
                            <Link to="/our-story" className="block text-on-surface hover:text-secondary font-bold tracking-wide transition-colors" onClick={() => setIsMobileMenuOpen(false)}>Our Story</Link>
                            <Link to="/video-consultation" className="block text-on-surface hover:text-secondary font-bold tracking-wide transition-colors" onClick={() => setIsMobileMenuOpen(false)}>Video Consult</Link>
                            <Link to="/contact" className="block text-on-surface hover:text-secondary font-bold tracking-wide transition-colors" onClick={() => setIsMobileMenuOpen(false)}>Contact</Link>
                        </div>
                    </nav>
                </div>
            </div>
            ...
            <a 
                ...
                className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-50 bg-[#25D366] text-white p-3 md:p-4 rounded-full shadow-[0_10px_30px_rgba(37,211,102,0.4)] hover:scale-110 hover:shadow-[0_15px_40px_rgba(37,211,102,0.6)] transition-all duration-300 flex items-center justify-center group"
