...
import { User, Mail, Phone, Package, Clock, MessageSquare, Edit3, LogOut, X, Star, Camera, Check } from 'lucide-react';
    ...
    const getFallbackStages = order => {
        ...
        return [
            ...
            },
            {
                key: 'in_transit',
                title: 'In Transit',
                description: isFulfilled ? 'Transit completed.' : 'Package moving through carrier network.',
                date: null,
    ...
    return (
        <main className="pt-24 pb-32 px-6 max-w-7xl mx-auto min-h-screen bg-[#fdfbf7] font-body">
            ...
            <header className="mb-20 flex flex-col md:flex-row md:items-end justify-between gap-8">
                <div>
                    ...
                    <p className="text-[#000000] text-xl max-w-2xl italic font-headline">
                        Welcome back, {(profileData.name || 'Patron').split(' ')[0]}. Your sartorial history and bespoke
                        journey await.
                    </p>
                </div>
                <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                ...
                <section className="md:col-span-4 p-10 rounded-[2.5rem] bg-white border border-[#d4af37]/10 shadow-[0_20px_50px_rgba(74,26,107,0.05)] flex flex-col justify-between">
                    <div>
                        <div className="flex items-center justify-between mb-10">
                            ...
                            <button
                                ...
                                className="text-[#d4af37] hover:scale-110 transition-transform p-2 bg-[#d4af37]/10 rounded-full font-display"
                            >
                    ...
                    <div className="mt-12">
                        {isEditing ? (
                            <button
                                onClick={handleSave}
                                className="w-full py-4 bg-primary text-[#fdfbf7] rounded-full font-bold uppercase tracking-widest text-xs hover:bg-[#3d1259] transition-all shadow-xl"
                            >
                                Save Changes
                            </button>
                        ) : (
                            <button
                                ...
                                className="w-full py-4 border border-primary text-primary rounded-full font-bold uppercase tracking-widest text-xs hover:bg-primary hover:text-[#fdfbf7] transition-all"
                            >
                                Update Details
                            </button>
                        )}
                        ...
                        {trackedOrder ? (
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 font-display">
                                <div className="space-y-8">
                                    <div className="flex items-center gap-6">
                                        ...
                                        <div>
                                            <h3 className="text-xl text-[#fdfbf7]">
                                                {trackedOrder.items[0]?.title || 'Bespoke Creation'}
                                            </h3>
                                            <p className="text-white/50 text-sm font-headline">
                                                Order #{trackedOrder.id} • {trackedOrder.date}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="relative pl-2 py-2">
                                        {(trackingDetails?.stages || getFallbackStages(trackedOrder)).map(
                                            (stage, idx, arr) => {
                                                ...
                                                return (
                                                    <div key={stage.key} className="relative pl-8 pb-6 last:pb-0">
                                                        ...
                                                        {idx < arr.length - 1 && (
                                                            <div
                                                                className={`absolute left-3 top-6 bottom-0 w-[2px] transition-colors duration-500
                                                            ${
                                                                isCompleted &&
                                                                (arr[idx + 1].status === 'completed' ||
                                                                    arr[idx + 1].status === 'active')
                                                                    ? 'bg-[#d4af37]'
                                                                    : 'bg-white/10'
                                                            }`}
                                                            />
                                                        ...
                                                        {/* Timeline node */}
                                                        <div
                                                            className={`absolute left-0 top-1 w-6 h-6 rounded-full flex items-center justify-center border transition-all duration-500 z-10
                                                        ${
                                                            isCompleted
                                                        ...
                                                        >
                                                            {isCompleted ? (
                                                                ...
                                                                <div
                                                                    className={`w-1.5 h-1.5 rounded-full ${isActive ? 'bg-[#d4af37]' : 'bg-current'}`}
                                                                />
                                                            )}
                                                        </div>
                                                        ...
                                                        {/* Content */}
                                                        <div
                                                            className={`transition-all duration-300 ${isPending ? 'opacity-40' : 'opacity-100'}`}
                                                        >
                                                            <div className="flex flex-wrap items-baseline gap-x-3 mb-1">
                                                                <span
                                                                    className={`font-display text-sm font-bold tracking-wider uppercase ${isActive ? 'text-[#d4af37]' : 'text-[#fdfbf7]'}`}
                                                                >
                                                                    {stage.title}
                                                                </span>
                                                                {stage.date && (
                                ...
                                <div className="p-8 bg-white/10 rounded-3xl border border-white/10 backdrop-blur-sm self-start">
                                    ...
                                    {trackingDetails?.tracking || trackedOrder.tracking ? (
                                        ...
                                        >
                                            ...
                                            Live Tracking #
                                            {trackingDetails?.tracking?.number || trackedOrder.tracking?.number}
                            ...
                            <div className="flex flex-col items-center justify-center py-10 text-center">
                                ...
                                <p className="text-white/60 italic max-w-sm">
                                    No bespoke creations in progress. When you secure a piece, its heritage journey will
                                    be tracked here.
                                </p>
                            </div>
                        )}
                ...
                <section className="md:col-span-12 p-10 rounded-[2.5rem] bg-white border border-[#d4af37]/10 shadow-[0_20px_50px_rgba(74,26,107,0.05)]">
                    <div className="flex items-center justify-between mb-10">
                        <h2 className="text-3xl text-primary">Sartorial History</h2>
                        {allOrders.length > 2 && (
                            <button
                                onClick={() => setShowAllOrders(!showAllOrders)}
                                className="text-[#d4af37] text-xs uppercase tracking-widest font-bold hover:text-primary transition-colors font-display"
                            >
                    ...
                    <div className="space-y-6">
                        {allOrders.length > 0 ? (
                            (showAllOrders ? allOrders : allOrders.slice(0, 3)).map(order => (
                                <div
                                    key={order.id}
                                    className="group p-6 rounded-[1.5rem] bg-[#fdfbf7]/50 hover:bg-white hover:shadow-[0_10px_30px_rgba(74,26,107,0.08)] transition-all duration-500 border border-transparent hover:border-[#d4af37]/20 flex items-center justify-between"
                                >
                                    <div className="flex items-center gap-6">
                                        ...
                                        <div className="space-y-2">
                                            <h4 className="text-lg text-primary font-medium line-clamp-1">
                                                {order.items[0]?.title}{' '}
                                                {order.items.length > 1 && `+ ${order.items.length - 1} more`}
                                            </h4>
                                            <p className="text-xs text-[#000000] tracking-wide font-display">
                                                {order.status.toUpperCase()} • {order.date}
                                            </p>
                                            <Link
                            ...
                            <div className="text-center py-16">
                                ...
                                <Link
                                    to="/catalog"
                                    className="inline-block px-10 py-3 bg-primary text-[#fdfbf7] rounded-full text-xs font-bold uppercase tracking-[0.2em] transition-all hover:bg-[#3d1259]"
                                >
