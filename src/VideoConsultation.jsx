...
export default function VideoConsultation() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.error(err);
            setIsSubmitting(false);
            showToast("Delivery failed. Please try again later.");
        }
    };
        ...
        <main className="min-h-screen bg-surface">
            {toast && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-primary text-white px-6 py-3 rounded-full shadow-xl z-[200] font-body text-sm z-[200]">
                {toast}
              </motion.div>
            )}
            {/* Hero Section */}
