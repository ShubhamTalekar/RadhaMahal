    ...
    const handlePlaceOrder = async () => {
        ...
        try {
            ...
            if (shopifyCartData?.cart?.checkoutUrl) {
                ...
                window.location.href = shopifyCartData.cart.checkoutUrl;
            } else {
        ...
        <main className="pt-12 pb-24 px-6 md:px-12 max-w-7xl mx-auto min-h-screen">
            {toast && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-primary text-white px-6 py-3 rounded-full shadow-xl z-[200] font-body text-sm">
                {toast}
              </motion.div>
            )}
