    ...
    try {
        ...
        return edges.map(({ node }) => {
            ...
            const videoSource =
                ...
                node.media?.edges.find(e => e.node.mediaContentType === 'VIDEO')?.node.sources?.[0];
...
export async function loginCustomer(email, password) {
    try {
        ...
        const customerData = await client.request(CUSTOMER_QUERY, {
        ...
        const parsedOrders = (customer.orders?.edges || []).map(edge => {
            ...
            return {
                ...
                items: (o.lineItems?.edges || []).map(li => ({
                    ...
                    id: li.node.variant?.product?.id?.split('/').pop() ?? null,
                })),
            };
        });
        ...
        return {
            customer: {
                ...
                orders: parsedOrders,
                addresses:
                    customer.addresses?.edges?.map(a => ({
                        id: a.node.id,
                        title: a.node.company || 'Address',
                        lines: [
                            a.node.address1,
                            a.node.address2,
                            `${a.node.city}, ${a.node.province} ${a.node.zip}`,
                            a.node.country,
                        ].filter(Boolean),
                        isPrimary: false,
                    })) || [],
